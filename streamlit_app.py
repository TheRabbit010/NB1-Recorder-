...
# 1. ตั้งค่า Page Config และฉีด CSS บังคับ Dark Mode
st.set_page_config(
...
# ฟังก์ชันคำนวณตำแหน่งคอลัมน์จาก Channel Index ของ Yokogawa DX2000
def get_ch_col_idx(ch_number):
    return 3 + ((ch_number - 1) * 2) + 1
...
def parse_single_file(uploaded_file):
    ...
    for i in range(1, 8):
        df[f"Top Zone #{i}"] = pd.to_numeric(raw_df[get_ch_col_idx(i)], errors="coerce")

    # CH008 - CH014: Bottom Zone #1 - #7
    for i in range(1, 8):
        ch_num = 7 + i  # CH008 ถึง CH014
        df[f"Bottom Zone #{i}"] = pd.to_numeric(raw_df[get_ch_col_idx(ch_num)], errors="coerce")

    # CH015: EXIT O2
    df["EXIT O2"] = pd.to_numeric(raw_df[get_ch_col_idx(15)], errors="coerce")

    # CH016: Dryer #1 & CH017: Dryer #2
    df["Dryer #1"] = pd.to_numeric(raw_df[get_ch_col_idx(16)], errors="coerce")
    df["Dryer #2"] = pd.to_numeric(raw_df[get_ch_col_idx(17)], errors="coerce")

    # CH018: N2 Flow Rate
    df["N2 Flow"] = pd.to_numeric(raw_df[get_ch_col_idx(18)], errors="coerce")

    # CH019: ENTRANCE O2
    df["ENTRANCE O2"] = pd.to_numeric(raw_df[get_ch_col_idx(19)], errors="coerce")

    # CH020: DEW POINT
    df["DEW POINT"] = pd.to_numeric(raw_df[get_ch_col_idx(20)], errors="coerce")
...
def process_multiple_files(uploaded_files):
    ...
    # รวมทุกไฟล์ ต่อเรียงตามเวลา และลบแถวซ้ำ
...
# ส่วน Sidebar สำหรับอัปโหลดไฟล์ (รับหลายไฟล์ได้)
...
if uploaded_files:
    try:
        ...
        st.sidebar.success(f"รวมข้อมูลสำเร็จ {len(uploaded_files)} ไฟล์ (รวม {len(raw_df)} แถว)")
        ...
        # 4. O2 & N2 Flow Rate (CH015, CH018, CH019) - Dual Axis
        if show_g4:
            st.subheader("4. ppmO2 Entry/Exit & N2 Flow (CH015, CH018, CH019)")
            fig4 = make_subplots(specs=[[{"secondary_y": True}]])
            
            # CH019 (ENTRANCE O2) - แกนซ้าย scale 0-200 ppm
        ...
        # 5. Dew Point (CH020) - Scale 10 to -100 °Cdp
        if show_g5:
        ...
        # ตารางข้อมูลและปุ่มดาวน์โหลดข้อมูลรวม
    ...
    # หน้า Welcome Screen มืดสวยงาม
    st.info("👈 กรุณาเลือกอัปโหลดไฟล์ (.csv หรือ .xlsx) ที่เมนูด้านซ้าย สามารถเลือกได้มากกว่า 1 ไฟล์")
    
    st.markdown("""
        <div style="background-color: #161b22; padding: 25px; border-radius: 10px; border: 1px solid #30363d;">
            <h3>📌 โครงสร้าง Channel ที่อัปเดตใหม่:</h3>
            <ul>
